...
module.exports = function({Classes,APIs, Logger, usingMocks}){
  ...
  awsApis.isAvailable = function(serviceString){
    return _fp.isObject(this[serviceString])
  }
  ...
  _fp.reduce((obj,item) => {
    ...
    if(_fp.isObject(item)){
      if(!_fp.has('name', item)){
        Logger.warn(msg.NO_NAME)
        return obj
      }

      prop = item.name
      if(_fp.has('options', item)){
        opts = item.options
      }
    }
    else if(_fp.isString(item)){
      prop = item
    }
    else {
